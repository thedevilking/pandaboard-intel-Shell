...
if [ $adhoc0_join -eq 1 ];then
	echo "insmod /system/fbaodv_intel.ko $MESH_DEV $AODV_NET $METRIC $RATE $GATEWAY aodv_blacklist=\"$aodvblacklist\" dtn_blacklist=\"$dtnblacklist\""
	insmod /system/fbaodv_intel.ko $MESH_DEV $AODV_NET $METRIC $RATE $GATEWAY aodv_blacklist="$aodvblacklist" dtn_blacklist="$dtnblacklist"
fi
